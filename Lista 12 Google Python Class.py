...
def not_bad(s):
  b = s.find('bad') 
  # acha 'bad' em s (não pode ser index pois index acha caractere)
  n = s.find('not')
  if b > n:
    s = s[:n] + 'good' + s[b+3:]
    #do começo á n + 'good' + de b+3(bad) para frente
  return s
...
def inicio_final(a, b):
  ...
  if len(a) % 2 == 1: #verifica se a string é ímpar
    a_metade = a_metade + 1 #se sim, adiciona um caractere a mais
  if len(b) % 2 == 1:
    b_metade = b_metade + 1
  return a[:a_metade] + b[:b_metade] + a[a_metade:] + b[b_metade:] 
  #adiciona do começo a metade de a e b + adiciona da metade ao final de a e b
